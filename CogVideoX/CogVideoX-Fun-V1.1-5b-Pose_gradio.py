...
STATE_FILE = "CogVideoX-Fun-V1.1-5b-Pose_state.json"
queue = []
...
def add_to_queue(control_video, guidance_scale, prompt, negative_prompt, height, width, num_inference_steps, fps, seed, use_dynamic_cfg):
    task = {
...
repo_id = "alibaba-pai/CogVideoX-Fun-V1.1-5b-Pose"
base_path = repo_id
files_to_download = [
        "model_index.json",
        "configuration.json",
...
def generate_video(control_video, guidance_scale, prompt, negative_prompt, height, width, num_inference_steps, fps, seed, use_dynamic_cfg):
    if seed == 0:
    ...
    os.makedirs("output_CogVideoX-Fun-V1.1-5b-Pose", exist_ok=True)
    output_path = f"./output_CogVideoX-Fun-V1.1-5b-Pose/{filename}"
...
with gr.Blocks() as demo:
    gr.Markdown("# CogVideoX-Fun v1.5 Pose")
    with gr.Tabs():
        with gr.Tab("Generate Video"):
            ...
            save_state_button.click(
                save_ui_state,
                inputs=[guidance_scale, prompt, negative_prompt, height, width, num_inference_steps, fps, seed, use_dynamic_cfg, memory_optimization],

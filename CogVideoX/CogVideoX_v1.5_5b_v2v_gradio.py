...
STATE_FILE = "CogVideoX_v1.5_5b_v2v_state.json"
queue = []
...
def add_to_queue(control_video, guidance_scale, prompt, negative_prompt, height, width, num_inference_steps, fps, seed, use_dynamic_cfg, strength):
    task = {
...
def save_ui_state(guidance_scale, prompt, negative_prompt, height, width, 
                  num_inference_steps, fps, seed, use_dynamic_cfg, memory_optimization, strength):
    state = {
        "guidance_scale": guidance_scale,
        "prompt": prompt,
        "negative_prompt": negative_prompt,
...
def generate_video(control_video, guidance_scale, prompt, negative_prompt, height, width, num_inference_steps, fps, seed, use_dynamic_cfg, strength):
    if seed == 0:
        seed = random.randint(0, 999999)
    processed_video = process_video_input(control_video)
    ...
    os.makedirs("output_CogVideoX_v1.5_5b_v2v", exist_ok=True)
    output_path = f"./output_CogVideoX_v1.5_5b_v2v/{filename}"
...
with gr.Blocks() as demo:
    gr.Markdown("# CogVideoX_v1.5_5b (Video to Video)")
    with gr.Tabs():
        with gr.Tab("Generate Video"):
            with gr.Row():
                control_video = gr.Video(label="Input Video")
            with gr.Row():
            ...
            with gr.Row():
                ...
                strength = gr.Slider(label="Strength", minimum=0, maximum=1, step=0.1, value=initial_state.get("strength", 0.8))
            ...
            generate_button.click(
                generate_video,
                inputs=[control_video, guidance_scale, prompt, negative_prompt, height, width, 
                       num_inference_steps, fps, seed, use_dynamic_cfg, strength],
                outputs=output_video
            )
            save_state_button.click(
                save_ui_state,
                inputs=[guidance_scale, prompt, negative_prompt, height, width, num_inference_steps, fps, seed, use_dynamic_cfg, memory_optimization, strength],
                outputs=gr.Text(label="State Status")
            )

        with gr.Tab("Batch Processing"):
            with gr.Row():
                batch_control_video = gr.Video(label="Input Video")
            with gr.Row():
            ...
            with gr.Row():
                ...
                batch_strength = gr.Slider(label="Strength", minimum=0, maximum=1, step=0.1, value=0.8)
            ...
            add_to_queue_button.click(
                add_to_queue,
                inputs=[batch_control_video, batch_guidance_scale, batch_prompt, batch_negative_prompt, batch_height,
                       batch_width, batch_num_inference_steps, batch_fps,
                       batch_seed, batch_use_dynamic_cfg, batch_strength],

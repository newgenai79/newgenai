...
STATE_FILE = "CogVideoX_v1.5_5B_t2v_state.json"
queue = []
...
def add_to_queue(guidance_scale, prompt, negative_prompt, height, width, num_frames, 
...
def generate_video(guidance_scale, prompt, negative_prompt, height, width, num_frames, num_inference_steps, fps, seed, use_dynamic_cfg):
    if seed == 0:
    ...
    os.makedirs("output_cogvideox_v1.5_5b_t2v", exist_ok=True)
    output_path = f"./output_cogvideox_v1.5_5b_t2v/{filename}"
...
with gr.Blocks() as demo:
    gr.Markdown("# CogVideoX v1.5-5B (text 2 video)")
    with gr.Tabs():
        with gr.Tab("Generate Video"):
            with gr.Row():
                prompt = gr.Textbox(label="Prompt", lines=7, 
                                  value=initial_state.get("prompt", "A panda, dressed in a small, red jacket and a tiny hat, sits on a wooden stool in a serene bamboo forest. The panda's fluffy paws strum a miniature acoustic guitar, producing soft, melodic tunes. Nearby, a few other pandas gather, watching curiously and some clapping in rhythm. Sunlight filters through the tall bamboo, casting a gentle glow on the scene. The panda's face is expressive, showing concentration and joy as it plays. The background includes a small, flowing stream and vibrant green foliage, enhancing the peaceful and magical atmosphere of this unique musical performance."))
        ...
        with gr.Tab("Batch Processing"):
            with gr.Row():
                batch_prompt = gr.Textbox(label="Prompt", lines=3,
                                        value="A batch of videos depicting different landscapes.")
            ...
            add_to_queue_button.click(
                add_to_queue,
                inputs=[batch_guidance_scale, batch_prompt, batch_negative_prompt, batch_height,
